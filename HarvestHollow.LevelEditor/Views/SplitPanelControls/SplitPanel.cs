...
public partial class SplitPanel : FrameworkElement
{
    ...
    public void SplitChild(int childIndex, UIElement element, double proportion)
    {
        ...
        SetSize(child, childSize);
        SetSize(element, elementSize);
    ...
    public void CombineChildren(int startIndex, int count)
    {
        ...
        UIElement child = Children[childIndex++];
        double combinedSize = (GetSize(child) + (Children.Splitters[0].Splitter_GetSize() * count));
        ...
        for (int combinedChildren = 0; combinedChildren < count; combinedChildren++)
        {
            ...
            combinedSize += GetSize(Children[childIndex]);
    ...
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        return ((Orientation == Orientation.Horizontal) ? measureHorizontal(availableSize) : measureVertical(availableSize));
    }
    ...
    private double getSplitSize(UIElement child)
    {
        ...
        double size = GetSize(child);

        // Are there any splitters in the panel?
        if (Children.Count > 1)
        {
            // Update the splitter size
            splitterSize = Children.Splitters[0].Splitter_GetSize();
        }
    ...
    private void starMeasure(double constraint)
    {
        // Iterate through the children
        foreach (UIElement child in Children)
        {
            // Update the star size
            _startUnitSize += GetSize(child);
        }

        // Is the star size valid?
        if (_startUnitSize < _epsilon)
        {
            // Set the star size to the width of the constraint
            _startUnitSize = constraint;

            // Set the size of the last child in the collection
            SetSize(Children.LastElement, _startUnitSize);
        }

        // Calculate the star size
        _startUnitSize = (constraint / _startUnitSize);
    }
    ...
    private void horizontalChildrenMeasure(Size constraint)
    {
        ...
        {
            // Set the constraint
            constraint.Width = (GetSize(child) * _startUnitSize);
    ...
    private void verticalChildrenMeasure(Size constraint)
    {
        // Initialize variables
        ...
        {
            // Set the constraint
            constraint.Height = (GetSize(child) * _startUnitSize);
    ...
    private double horizontalChildArrange(UIElement child, double offset, double height)
    {
        ...
        double width = (GetSize(child) * _startUnitSize);
    ...
    private double verticalChildArrange(UIElement child, double offset, double width)
    {
        ...
        double height = (GetSize(child) * _startUnitSize);
    ...
    private Size measureVertical(Size constraint)
    {
        ...
        // Measure the children
        verticalChildrenMeasure(constraint);
    ...
    // Method is part of an attached property definition in WPF.
    // Alternate naming convention is required to match the property name.
